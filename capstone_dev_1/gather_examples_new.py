...
# opening CSV file to append trained name
def append_new_user_as_row(dict_of_elem):
    file_name = 'trained_name.csv'
    field_names = ['Name', 'Updated_on']
    reader_type = ''
    ...
    with open(file_name, reader_type, newline='') as write_obj:
        ...
        assert isinstance(dict_of_elem, object)
...
for file in files:
    ...
    if 'fake' in name.lower():
        name = 'Fake'
    print(name)
    # adding user name to CSV file
    row_dict = {'Name': name, 'Updated_on': datetime.now()}
    append_new_user_as_row(row_dict)

    BASE_DIR = r'C:/anton/py/capstone_dev_1/dataset/'
